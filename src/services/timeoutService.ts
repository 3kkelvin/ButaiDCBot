...
export class TimeoutService {
  ...
  public async jailSingleChannel(
    ...
    return await lockService.runWithLock({ lockKey, ttlMs: 15000 }, async () => {
      ...
      await DiscordLogger.sendInfoLog({
        title: '[Timeout] 單頻道禁言生效',
        message: `<@${executor.id}> 在 <#${targetChan.id}> 禁言了 <@${target.id}> ${minutes} 分鐘 (原因: ${reason})`,
        guildId: guild.id,
        userId: executor.id,
  ...
  public async jailGlobal(
    ...
    return await lockService.runWithLock({ lockKey, ttlMs: 15000 }, async () => {
      ...
      const majorRoleIds = [config.roles.voter, config.roles.official, config.roles.temporary].filter(
        Boolean
      );
